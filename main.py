...
learn_date='2018-05-01'

stock_datas=sdl.all_stock_data_load(stock_names)

train_essential_index=es.get_essential_index(stock_datas,14,learn_date)
    ...
    if(item['date']>=learn_date):
        ...
        if(prediction[0]>=105):
            ...
            if(len(stock_datas[item['key']])>(item['idx']+1)):
                ...
                print("싯가대비상승률 : %f" %(float(stock_datas[item['key']][item['idx']+1][4])/float(stock_datas[item['key']][item['idx']+1][3])))
                money=money*float(stock_datas[item['key']][item['idx']+1][4])/float(stock_datas[item['key']][item['idx']+1][3])
                print("익일상승률 : %f" %(float(stock_datas[item['key']][item['idx']+1][4])/float(stock_datas[item['key']][item['idx']][4])))
